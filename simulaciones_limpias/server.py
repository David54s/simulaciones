...
def multinomial_3d():
    n = request.json.get('n', 1)  # Lanzamientos por experimento
    max_repeticiones = request.json.get('max_repeticiones', 100)  # Máximo número de experimentos
    puntos_muestra = request.json.get('puntos_muestra', 20)  # Puntos a muestrear
    
    caras = [1, 2, 3, 4, 5, 6]
    
    # Calcular puntos de muestreo sin NumPy
    repeticiones_points = []
    if puntos_muestra == 1:
        repeticiones_points = [max_repeticiones]
    else:
        step = max(1, (max_repeticiones - 10) // (puntos_muestra - 1))
        repeticiones_points = [10 + i * step for i in range(puntos_muestra)]
        # Asegurarse de que el último punto sea max_repeticiones
        if repeticiones_points[-1] > max_repeticiones:
            repeticiones_points[-1] = max_repeticiones
    ...
    for repeticiones in repeticiones_points:
        ...
        for cara in caras:
            datos_3d['repeticiones'].append(repeticiones)
            datos_3d['caras'].append(cara)
            datos_3d['frecuencias'].append(freq_acumulada[cara])
            datos_3d['probabilidades'].append(freq_acumulada[cara] / total_lanzamientos)
