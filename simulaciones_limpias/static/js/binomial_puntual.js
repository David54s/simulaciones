...
btnGenerarBinomial.addEventListener('click', async () => {
    ...
    try {
        ...
        const resultadosAMostrar = data.resultados.slice(0, 100);
        const totalMostrados = resultadosAMostrar.length;

        let htmlResultados = `
            <div class="resumen-resultados">
                <p class="resumen-text">
                    Mostrando los primeros ${totalMostrados} resultados de ${n} intentos totales
                </p>
            </div>
        ...
        resultadosAMostrar.forEach((resultado, index) => {
            const clase = resultado === "1" ? "resultado-exito" : "resultado-fracaso";
            const texto = resultado === "1" ? "Éxito" : "Fracaso";
            const numero = index + 1;
            
            htmlResultados += `
                <div class="${clase} resultado-item" title="Intento ${numero}: ${texto}">
                    ${numero}: ${texto === "Éxito" ? "C" : "X"}
                </div>
            `;
        });
        ...
        document.getElementById('download-section-detailed').style.display = 'block';
...
// Funcionalidad de descarga para el botón detallado
document.getElementById('btnDescargarDetallado').addEventListener('click', () => {
    ...
    link.download = 'resultados_binomial_puntual.txt';
