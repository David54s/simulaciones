...
resultadosAMostrar.forEach((valor, index) => {
    ...
    // Clasificar valores según su relación con la media
    const clase = valor >= mu ? "resultado-sobre-media" : "resultado-bajo-media";
...
document.getElementById('btnDescargar').addEventListener('click', () => {
    ...
    let contenido = '======== SIMULACION NORMAL CON PARAMETROS ========\n\n';
    contenido += `Parámetros: n=${parametrosSimulacion.n}, μ=${parametrosSimulacion.mu}, σ=${parametrosSimulacion.sigma}\n\n`;
    
    // Usar las estadísticas ya calculadas por el backend
    const media_muestral = resultadosSimulacion.reduce((a, b) => a + b, 0) / resultadosSimulacion.length;
    const varianza_muestral = resultadosSimulacion.reduce((a, b) => a + Math.pow(b - media_muestral, 2), 0) / resultadosSimulacion.length;
    
    contenido += 'ESTADÍSTICAS:\n';
    contenido += `Media muestral: ${media_muestral.toFixed(4)}    Media teórica: ${parametrosSimulacion.mu.toFixed(4)}\n`;
    contenido += `Varianza muestral: ${varianza_muestral.toFixed(4)}    Varianza teórica: ${(parametrosSimulacion.sigma ** 2).toFixed(4)}\n\n`;
    ...
    link.download = 'resultados_normal_parametros.txt';
